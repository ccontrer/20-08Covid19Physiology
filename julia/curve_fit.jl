# Modified code from 
# https://github.com/JuliaNLSolvers/LsqFit.jl/blob/master/src/LsqFit.jl
...
#     # results = levenberg_marquardt(R, p0; kwargs...) replace with optimize from Optim
#     p = minimizer(results)
#     return LsqFitResult(p, value!(R, p), jacobian!(R, p), converged(results), wt)
# end

function lmfit(f, l::AbstractArray, u::AbstractArray, p0::AbstractArray, wt::AbstractArray, args...; kwargs...)
    # this is a convenience function for the curve_fit() methods
    # which assume f(p) is the cost functionj i.e. the residual of a
    # model where
    #   model(xpts, params...) = ydata + error (noise)
    ...
    # construct Jacobian function, which uses finite difference method
    res = TwiceDifferentiable((p) -> sum(abs2, f(p)), ones(length(p0)))
    result = optimize(res, l, u, p0, args...; kwargs...)
    p = Optim.minimizer(result)
    # TODO: Fix hessian
    H = hessian!(res, p)
    conv = Optim.converged(result)
    return LsqFitResult(p, f(p), H, conv, wt, result)
end
...
function curve_fit(model, xdata::AbstractArray, ydata::AbstractArray, l::AbstractArray, u::AbstractArray, p0::AbstractArray, args...; kwargs...)
    check_data_health(xdata, ydata)
    # construct the cost function
    T = eltype(ydata)

    f = (p) -> model(xdata, p) - ydata
    lmfit(f, l, u, p0, T[], args...; kwargs...)
end
